...
# WAF v2 requires a dedicated subnet with no NSG restrictions on AppGW ports
...
# ── Subnet: Frontend VNet Integration ─────────────────────────
...
# ── Subnet: Backend VNet Integration ──────────────────────────
...
resource "azurerm_subnet" "private_ep" {
  name                 = "snet-private-ep"
  resource_group_name  = var.resource_group_name
  virtual_network_name = azurerm_virtual_network.main.name
  address_prefixes     = [var.subnet_private_ep_cidr]

  # Required for private endpoints (use new attribute for azurerm >= 3.65)
...
resource "azurerm_network_security_group" "backend" {
  ...
  # Allow VNet internal communication (App Gateway → Backend)
  security_rule {
...
# ── Private DNS Zone: Cosmos DB (MongoDB API) ──────────────────
